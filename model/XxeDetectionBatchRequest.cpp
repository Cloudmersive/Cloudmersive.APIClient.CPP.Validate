...
void XxeDetectionBatchRequest::validate()
{
    // TODO: implement validation
}

web::json::value XxeDetectionBatchRequest::toJson() const
{
...
void XxeDetectionBatchRequest::fromJson(web::json::value& val)
{
    {
        ...
        for( auto& item : val[utility::conversions::to_string_t("RequestItems")].as_array() )
        {
            if(item.is_null())
            {
                m_RequestItems.push_back( std::shared_ptr<XxeDetectionRequestItem>(nullptr) );
            }
            else
            {
                std::shared_ptr<XxeDetectionRequestItem> newItem(new XxeDetectionRequestItem());
...
void XxeDetectionBatchRequest::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
void XxeDetectionBatchRequest::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t("."))
    {
...
std::vector<std::shared_ptr<XxeDetectionRequestItem>>& XxeDetectionBatchRequest::getRequestItems()
{
    return m_RequestItems;
}

void XxeDetectionBatchRequest::setRequestItems(std::vector<std::shared_ptr<XxeDetectionRequestItem>> value)
{
    m_RequestItems = value;
    m_RequestItemsIsSet = true;
}
bool XxeDetectionBatchRequest::requestItemsIsSet() const
{
    return m_RequestItemsIsSet;
}

void XxeDetectionBatchRequest::unsetRequestItems()
{
    m_RequestItemsIsSet = false;
}
